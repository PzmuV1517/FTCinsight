...
const PageMatchTable = ({ data }: { data: MatchData }) => {
  ...
  const redEventAuto =
    (red1EventData?.auto_points ?? 0) + (red2EventData?.auto_points ?? 0);
  const blueEventAuto =
    (blue1EventData?.auto_points ?? 0) + (blue2EventData?.auto_points ?? 0);
  const redEventTeleop =
    (red1EventData?.teleop_points ?? 0) + (red2EventData?.teleop_points ?? 0);
  const blueEventTeleop =
    (blue1EventData?.teleop_points ?? 0) + (blue2EventData?.teleop_points ?? 0);
  const redEventFouls = redPredTotal - (redEventAuto + redEventTeleop);
  const blueEventFouls = bluePredTotal - (blueEventAuto + blueEventTeleop);

  // Used for completed events

  const redMatchAuto = (red1Data?.auto_points ?? 0) + (red2Data?.auto_points ?? 0);
  const blueMatchAuto = (blue1Data?.auto_points ?? 0) + (blue2Data?.auto_points ?? 0);
  const redMatchTeleop =
    (red1Data?.teleop_points ?? 0) + (red2Data?.teleop_points ?? 0);
  const blueMatchTeleop =
    (blue1Data?.teleop_points ?? 0) + (blue2Data?.teleop_points ?? 0);
  const redMatchFouls = redPredTotal - (redMatchAuto + redMatchTeleop);
  const blueMatchFouls = bluePredTotal - (blueMatchAuto + blueMatchTeleop);
